...
class UserManager(BaseUserManager):
    ...
    def _create_user(self, username, phone_number, email, password, is_staff, is_superuser, **extra_fields):
        #creates and saves an user whit the given username,email and password
        now = timezone.now()
        if not username:
    ...
    def create_user(self, username=None, phone_number=None, email=None, password=None, **extra_fields):
        if username is None:
            ...
            if phone_number:
                username = random.choice('abcdefghijklmopqrstuvwxyz')+ str(phone_number)[-7:]
...
class User (AbstractBaseUser,PermissionsMixin):
    ...
    username = models.CharField (_('username'), max_length=30, unique=True,
                help_text = _('Required.30charecters or fewer string with a latter. letters, digits and @/./+/-/_ only.'),
                validators = [validators.RegexValidator(r'^[a-zA-Z][a-zA-Z0-9_\.]+$',
                _('Enter a valid username string with a-z.this value may contain only letters,number and underscore characters.'), 'invalid'),],
    ...
    is_active = models.BooleanField (_('active'), default= True,
                help_text = _('designates whether this user shuld be treated as active.unselect this instead of deleting acconts'))
    ...
    def get_full_name(self) :
        #return the first_name  plus last_name with a space betwean.
    ...
    def email_user(self, object, messages, from_email=None, **kwargs):
        # send an email to this user
        send_mail(object, messages, from_email, [self.email], **kwargs)

    @property
    def is_loggedin_user(self) :
        # return true if user has acthally logged in with valid ceredntials.  
...
class UserProfile(models.Model):
    ...
    nike_name = models.CharField(_('nike_name'), max_length=150, blank=True)
    avatar = models.ImageField(_('avatar'), blank=True)
    birthday = models.DateField(_('birthday'), null=True, blank=True,)
    gender = models.BooleanField(_('gender'), help_text=_('femaleis false ,male is true,null is unset '))
    ...
    #            validators= [validators.RegexValidator(r'^989[0-3,9]\d{8}$', _('Enter a vaild mobile number.'),'invalid')],)    
